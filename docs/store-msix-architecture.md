...
On 2026-09-15, manual GitHub Actions run [`34945022858`](https://github.com/kleedaisuki/scrap/actions/runs/34945022858) produced artifact `10386584492` with the exact production identity, Store version `1.0.2.0`, and product version `0.2.0-preview.8`. The downloaded unsigned MSIX has SHA-256 `67dccedacc39c3207384526cc9592d577ec2aae5abb9be36fcddb1c5cc9ed989`. WACK `10.0.26100.8249` was then run against that same file: it completed all 24 tests (`PARTIAL_RUN=FALSE`) and reported `OVERALL_RESULT=PASS`; every mandatory test passed. The preserved XML report has SHA-256 `8d0295891d55609579dc91a7cb8d1effc0403040ee0836f54ff1645e27362a86`. Its optional **Blocked executables** analyzer reported `FAIL` because the self-contained .NET payload contains process-launch API imports and runtime/tool-name strings. The wrapper surfaces this finding even though WACK's own roll-up remains `PASS`.
