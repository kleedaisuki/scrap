...
**Eligibility and cost constraint (rechecked 2026-09-15):** Microsoft's current comparison limits Azure Artifact Signing to organizations in the US, Canada, EU, and UK, and to individuals in the US and Canada. It lists conventional OV certificates at roughly USD 150–300/year and notes that post-2023 private keys normally require an HSM or hardware token. That makes the repository's exportable-PFX secret lane a compatibility path for an already suitable credential, not the recommended way to acquire a new one. Provider-hosted signing needs a provider-specific Action; a hardware token needs a deliberately operated self-hosted runner ([Microsoft code-signing options](https://learn.microsoft.com/en-us/windows/apps/package-and-deploy/code-signing-options)).

For this GPL-3.0 project, the most plausible direct-download path is **SignPath Foundation Open Source Code Signing**. It is free for accepted projects and supports deep-signing the PE files inside an MSI before signing the MSI itself ([Foundation conditions](https://signpath.org/terms.html), [deep-signing syntax](https://docs.signpath.io/artifact-configuration/syntax)). Acceptance is discretionary: the project must already be released, documented, actively maintained, entirely open source, operated with MFA and explicit signing roles, and have enough verifiable reputation for the Foundation to publish under its identity. Scrap now records the required policy and uniform product metadata, but its very young preview-only history makes acceptance uncertain rather than guaranteed.

The deterministic no-SmartScreen path is a **Microsoft Store MSIX** submission: Microsoft re-signs an accepted MSIX and currently describes Store developer registration and signing as free. This requires the owner to complete identity verification, reserve the app name, and provide the Partner Center package identity before a final manifest can be built. Store MSIX and SignPath therefore form complementary tracks: Store for the lowest-friction consumer install, SignPath for a trusted GitHub-direct MSI if the project is accepted. Do not publish a nominal “signed” release using a self-signed certificate; Microsoft treats that like unsigned software for SmartScreen reputation ([publishing guide](https://learn.microsoft.com/en-us/windows/apps/package-and-deploy/publish-first-app)).
